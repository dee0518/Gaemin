const header = () => {
  ...
  const $addAddressBtn = document.querySelector('.modal__body__add__address__btn');
  ...
  $modal.addEventListener('click', e => {
    if (e.target.closest('.modal__body') && !e.target.classList.contains('modal__body__close__btn')) return;
  ...
  $myPlaceList.addEventListener('click', e => {
    ...
    modalOff();
    $ModalOpenBtn.textContent = e.target.closest('li').firstElementChild.textContent;
  });
